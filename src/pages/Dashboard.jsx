...
export default function Dashboard() {
  // SAFE SELECTORS (Prevents white screen crash)
  const user = useSelector((state) => state?.auth?.user || null);
  const favIds = useSelector((state) => state?.favorites?.ids || []);
  const artworks = useSelector((state) => state?.artworks?.items || []);

  const role = user?.role || "Visitor"; // fallback role
  const items = blocks[role] || blocks.Visitor;

  // SAFE favourites
  const favs = Array.isArray(artworks)
    ? artworks.filter((a) => favIds.includes(a.id))
    : [];
